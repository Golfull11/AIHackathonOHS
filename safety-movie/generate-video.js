...
async function main() {
    ...
    for (const categoryDoc of categories) {
        ...
        // タスク1：descriptionの動画
        if (categoryData.description && categoryData.description.trim() !== "" && categoryData.description !== "生成失敗") {
            videoTasks.push({ text: categoryData.description, type: 'description' });
        } else {
            console.log("  - Skipping 'description' video (content is empty or failed).");
        }

        // タスク2：measuresの最初の項目の動画
        if (categoryData.measures && categoryData.measures.length > 0 && categoryData.measures[0].trim() !== "" && categoryData.measures[0] !== "生成失敗") {
            videoTasks.push({ text: categoryData.measures[0], type: 'measure_1' });
        } else {
            console.log("  - Skipping 'measure_1' video (content is empty or failed).");
        }
        
        if (videoTasks.length === 0) {
            console.log("  No valid content to generate videos for this category. Skipping.");
            continue;
        }

        // 既存の動画URLを保持しつつ、新しいURLで上書きするためのオブジェクト
        ...
        // 4. Firestoreドキュメントを動画URLで更新
        if (Object.keys(videoUrls).length > 0) {
            // .update()ではなく.set(..., { merge: true }) を使うと、
            // videoUrlsフィールドがない場合でも安全に書き込める
